...
<div class="max-w-6xl mx-auto px-4 py-6 md:py-12">
    ...
    ?>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-14 items-start">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <div class="mt-2 text-2xl md:text-3xl font-bold text-[#4A2B43]">
                        <?php echo $price_html; ?>
                    </div>
                </div>
                ...
                <!-- Dynamic WooCommerce Native Cart & Variations Form -->
                ...
                <!-- Quick Order Hub (Order However You Prefer) -->
                <div class="bg-[#FAF8F5] rounded-2xl p-4 md:p-5 border border-[#F0ECE1] space-y-3.5 shadow-sm">
                    <div class="flex items-center gap-2 text-xs font-bold text-brandDark">
                        <i data-lucide="headphones" class="w-4 h-4 text-[#B5945A]"></i>
                        <span>আপনার যেভাবে সুবিধা, সেভাবেই অর্ডার করুন:</span>
                    </div>

                    <!-- Button Grid based on Admin Panel Options -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        
                        <!-- 1. Direct Order Modal Button -->
                        <button type="button" onclick="prepareAndOpenModal()" class="flex items-center gap-3 p-3.5 rounded-2xl bg-[#133E32] text-white hover:bg-[#0e2f26] transition active:scale-[0.98] shadow-sm text-left">
                            ...
                            <div>
                                ...
                                <span class="block text-[10px] text-neutral-300"><?php echo esc_html($quick_order_sublabel); ?></span>
                            </div>
                        </button>

                        <!-- 2. WhatsApp Order Button -->
                        <?php if ($show_wa == '1') : ?>
                            <a href="<?php echo esc_url($whatsapp_url); ?>" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 p-3.5 rounded-2xl bg-[#E8F8EE] text-[#1E7B44] border border-[#CDEED8] hover:bg-[#ddf5e5] transition active:scale-[0.98] text-left">
                        ...
                        <?php if ($show_phone == '1') : ?>
                            <a href="<?php echo esc_url($call_url); ?>" class="flex items-center gap-3 p-3.5 rounded-2xl bg-[#F5EFE6] text-brandDark border border-[#E8DFC9] hover:bg-[#ebe2d4] transition active:scale-[0.98] text-left">
                                <div class="w-9 h-9 rounded-xl bg-white flex items-center justify-center shrink-0 shadow-sm text-[#B5945A]">
                                    <i data-lucide="phone-call" class="w-5 h-5"></i>
                                </div>
                                <div>
                                    <span class="block text-xs font-bold leading-tight"><?php echo esc_html($phone_label); ?></span>
                                    <span class="block text-[11px] font-semibold text-neutral-600"><?php echo esc_html($phone_number); ?></span>
                                </div>
                            </a>
                        <?php endif; ?>

                        <!-- 4. Messenger Order Button -->
                        <?php if ($show_messenger == '1') : ?>
                            <a href="<?php echo esc_url($messenger_url); ?>" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 p-3.5 rounded-2xl bg-[#EEF4FF] text-[#0064E0] border border-[#D5E3FF] hover:bg-[#e0ecff] transition active:scale-[0.98] text-left">
                                <div class="w-9 h-9 rounded-xl bg-white flex items-center justify-center shrink-0 shadow-sm">
                                    <svg class="w-5 h-5 text-[#0084FF] fill-current" viewBox="0 0 24 24">
                                        <path d="M12 2C6.36 2 2 6.13 2 11.7c0 2.91 1.19 5.43 3.16 7.17v3.52l3.35-1.84c1.07.3 2.21.46 3.49.46 5.64 0 10-4.13 10-9.7S17.64 2 12 2zm1.06 13.06l-2.67-2.85-5.21 2.85 5.73-6.09 2.74 2.85 5.14-2.85-5.73 6.09z"/>
                                    </svg>
                                </div>
                                <div>
                                    <span class="block text-xs font-bold leading-tight"><?php echo esc_html($messenger_label); ?></span>
                                    <span class="block text-[10px] text-neutral-500"><?php echo esc_html($messenger_sublabel); ?></span>
                                </div>
                            </a>
                        <?php endif; ?>
        ...
        <div class="md:hidden fixed bottom-16 left-0 right-0 z-40 bg-white/95 backdrop-blur-lg border-t border-brandBorder px-4 py-2.5 flex items-center justify-between shadow-2xl">
            <div>
                <span class="text-[10px] text-brandGrayText block">মূল্য</span>
                <span class="text-base font-bold text-brandDark"><?php echo $price_html; ?></span>
            </div>
            <div class="flex items-center gap-2">
                <?php if ($show_wa == '1') : ?>
                    <a href="<?php echo esc_url($whatsapp_url); ?>" target="_blank" class="w-9 h-9 rounded-full bg-[#E8F8EE] text-[#1E7B44] border border-[#CDEED8] flex items-center justify-center">
                        <svg class="w-4 h-4 text-[#25D366] fill-current" viewBox="0 0 24 24">
                            <path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.019 3.287l-.582 2.128 2.182-.573c.978.58 1.911.928 3.145.929 3.178 0 5.767-2.587 5.768-5.766.001-3.187-2.575-5.771-5.764-5.771zm3.392 8.244c-.144.405-.837.774-1.17.824-.312.045-.694.072-2.146-.531-1.859-.772-3.053-2.66-3.146-2.784-.093-.123-.746-.994-.746-1.895 0-.901.472-1.344.64-1.528.168-.184.367-.23.49-.23.123 0 .245.001.352.006.113.005.265-.043.414.316.153.368.523 1.277.569 1.37.046.093.076.2.015.323-.061.123-.092.2-.184.307-.092.108-.194.24-.277.323-.092.092-.188.192-.081.376.107.184.478.788 1.025 1.275.706.628 1.301.822 1.486.914.184.092.292.077.4-.046.108-.123.46-0.537.583-.721.123-.184.246-.153.414-.092.169.061 1.074.507 1.258.6.184.092.307.138.353.215.046.077.046.446-.098.851zM12 2C6.477 2 2 6.477 2 12c0 1.891.524 3.66 1.434 5.176L2 22l4.957-1.301A9.957 9.957 0 0012 22c5.523 0 10-4.477 10-10S17.523 2 12 2z"/>
                        </svg>
                    </a>
                <?php endif; ?>
                <button type="button" onclick="prepareAndOpenModal()" class="px-5 py-2.5 bg-[#133E32] text-white rounded-full text-xs font-bold hover:bg-[#0e2f26] transition active:scale-95 shadow-md flex items-center gap-1.5">
            ...
            <div class="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-brandBorder relative overflow-hidden max-h-[92vh] overflow-y-auto">
                ...
                <div id="orderFormView">
                    ...
                    <form id="ukazQuickOrderForm" class="space-y-3">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-2 text-xs font-medium">
                                <label class="flex items-center gap-2 p-2.5 rounded-xl border border-brandBorder cursor-pointer">
                                    <input type="radio" name="delivery_area" value="inside" checked class="accent-[#133E32]">
                                    <span>ঢাকার ভিতরে (৳<?php echo esc_html($delivery_inside); ?>)</span>
                                </label>
                                <label class="flex items-center gap-2 p-2.5 rounded-xl border border-brandBorder cursor-pointer">
                                    <input type="radio" name="delivery_area" value="outside" class="accent-[#133E32]">
                        ...
                        <button type="submit" id="submitOrderBtn" class="w-full py-3.5 bg-[#133E32] text-white font-bold rounded-xl text-sm hover:bg-[#0e2f26] transition flex items-center justify-center gap-2">
                ...
                <!-- 2. Apple-Style Order Success Screen (Hidden by Default) -->
...
<script>
    ...
    if (orderForm) {
        orderForm.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    // Show dedicated apple-style success view
                    ...
                    // Configure WhatsApp Instant Confirm button
                    ...
                    // Re-render Lucide icons for check-circle
